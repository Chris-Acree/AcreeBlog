...
namespace AcreeBlog.Controllers
{
    public class HomeController : Controller
  {
    ...
    [Route("blogpostbyslug/{slug}")]
        [Route("home/blogpostbyslug/{slug}")]
        public async Task<IActionResult> BlogPostBySlug(string slug)
    {
        ...
        //var blogPostViewModel = _mapper.Map< AcreeBlog.ViewModels.Home.BlogPostViewModel>(bpvm);
        var blogPostViewModel = new AcreeBlog.ViewModels.Home.BlogPostViewModel()
        {
            Id = bpvm.Id,
            Slug = bpvm.Slug,
            Title = bpvm.Title,
            Author = bpvm.Author,
            Description = bpvm.Description,
            Content = bpvm.Content,
            ModifiedAt = bpvm.ModifiedAt,
            PublishOn = bpvm.PublishOn,
            Categories = bpvm.Categories,
            Topics = bpvm.Topics,
            Comments = bpvm.Comments,
            Image = bpvm.Image,
            IsHtml = bpvm.IsHtml,
            RowGuid = bpvm.RowGuid
        };

        if (!bpvm.IsHtml) bpvm.Content = CommonMarkConverter.Convert(bpvm.Content);

        return View("BlogPost", blogPostViewModel);
    }

    // Depricated
    [Route("blog_old/{slug}")]
    public async Task<IActionResult> BlogPostBySlug_Old(string slug)
    {
      if (slug == null)
      {
        _logger.LogWarning("Warning - BlogPost with null slug called.");
      ...
      return View("BlogPostBySlug", bpvm);
    }
